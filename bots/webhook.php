...
Do Your Query/Process, then insert the result to Outbox/Multi table using API 
...
function sendMessage($wa_profil, $wa_no, $wa_text) {
  echo $wa_text ;  // For Testing in BROWSER, checking is BOT Reply or Not ?
  $token = '1demo' ; // CHANGE WITH YOUR TOKEN 
  $text = urlencode( $wa_text ) ;
  $url =  "http://localhost/api/send.php?token=$token&no=$wa_no&text=$text" ;
  $ch = curl_init();      
